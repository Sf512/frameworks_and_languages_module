...
app.get('/items/', function (req, res) {
    validItems = model.searchOne(req.query ["user_id"], req.query["keywords"],req.query["lat"], req.query["lon"], req.query["radius"], req.query["date_from"], req.query["date_to"])
    res.status(200).send(validItems)
})

app.delete('/item/:itemId', function(req,res){
    delvar = model.delete_item(req.params["itemID"])
    if(delvar){
